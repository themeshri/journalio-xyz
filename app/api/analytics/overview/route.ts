...
const checkRateLimit = rateLimitByUser({ limit: 60, windowSeconds: 60, prefix: 'analytics-overview' })
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `overview:${userId}:${params.addresses.join(',')}:${params.dexes.join(',')}:${searchParams.get('startDate') || ''}:${searchParams.get('endDate') || ''}`
